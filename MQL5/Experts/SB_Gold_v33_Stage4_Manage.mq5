//+------------------------------------------------------------------+
//|                                SB_Gold_v33_Stage4_Manage.mq5     |
//|                  Asian Session Range Breakout — XAUUSD           |
//|                                                                  |
//|  ╔══════════════════════════════════════════════════════════╗    |
//|  ║  BOSQICH 4 / 5 :  POZITSIYA BOSHQARUVI                   ║    |
//|  ╚══════════════════════════════════════════════════════════╝    |
//|                                                                  |
//|  Bosqich 1 + 2 + 3 ning HAMMASI + quyidagilar:                   |
//|                                                                  |
//|  [22] BREAK-EVEN. +1R da SL zararsiz zonaga ko'chiriladi.        |
//|       Eski kodda savdo +1.8R ga borib, keyin -1R ga qaytishi     |
//|       mumkin edi (2.8R yo'qotish). Endi eng yomoni +0.15R.       |
//|                                                                  |
//|  [23] TRAILING STOP. +1.5R dan keyin SL narx ortidan ergashadi.  |
//|       Katta harakatlardan to'liq foyda olish imkoni.             |
//|                                                                  |
//|  [24] QISMAN YOPISH (ixtiyoriy). +1R da hajmning yarmi           |
//|       yopiladi, qolgani trailing bilan ketadi.                   |
//|                                                                  |
//|  [25] VAQT BO'YICHA MAJBURIY CHIQISH. Eski kodda pozitsiya       |
//|       SL/TP gacha KUNLAB ochiq qolardi — tungi va HAFTA OXIRI    |
//|       GAP riski (gap da SL kafolatlanmaydi, zarar rejadagidan    |
//|       2-3 barobar ko'p bo'lishi mumkin).                         |
//|                                                                  |
//|  [26] JUMA KUNI MAJBURIY YOPISH — hafta oxiri gapidan himoya.    |
//|                                                                  |
//|  [27] MAX USHLAB TURISH VAQTI (soat).                            |
//|                                                                  |
//|  [28] Boshlang'ich SL GlobalVariable da saqlanadi -> terminal    |
//|       qayta yuklansa ham BE/trailing to'g'ri ishlaydi.           |
//|                                                                  |
//|  MUHIM: pozitsiya boshqaruvi HAR TICKDA ishlaydi (NewBar         |
//|  gate dan OLDIN), aks holda BE/trailing kechikardi.              |
//+------------------------------------------------------------------+
#property copyright "Session Breakout Gold - Stage 4"
#property version   "3.30"
#property description "BOSQICH 4/5: Break-even, trailing, qisman yopish,"
#property description "vaqt bo'yicha chiqish, hafta oxiri himoyasi."
...
input int      InpTrailStepPoints   = 20;     // Min SL ko'chirish qadami (punkt)

input group "════════ QISMAN YOPISH  [BOSQICH 4] ════════"
input bool     InpUsePartialClose   = false;  // Qisman yopish yoqilsinmi
...
input bool     InpCloseBeforeWeekend= true;   // Juma kuni majburiy yopish
...
ulong    g_prevTicket     = 0;    // GV tozalash uchun
...
//==================================================================//
//   [28] BOSHLANG'ICH SL NI SAQLASH (restart-safe, GlobalVariable) //
//==================================================================//
...
void   StoreInitialSL(const ulong ticket, const double sl)
  { GlobalVariableSet(GVSLName(ticket), sl); }

bool   HasInitialSL(const ulong ticket)
  { return GlobalVariableCheck(GVSLName(ticket)); }

double GetInitialSL(const ulong ticket)
  { return GlobalVariableGet(GVSLName(ticket)); }

void   MarkPartialDone(const ulong ticket)
  { GlobalVariableSet(GVPPName(ticket), 1.0); }

bool   IsPartialDone(const ulong ticket)
  { return GlobalVariableCheck(GVPPName(ticket)); }

//--- pozitsiya yopilganda barcha GV larni tozalash --------------------
void CleanupGV()
  {
      ...
      if(done >= InpMaxTradesPerDay)
        {
         if(g_blockedDay != today)
           {
            g_blockedDay = today;
            PrintFormat("[LIMIT] Kunlik savdo limiti: %d/%d", done, InpMaxTradesPerDay);
           }
         return false;
        }
     }
            ...
            if(lossPct >= InpMaxDailyLossPct)
              {
               if(g_blockedDay != today)
                 {
                  g_blockedDay = today;
                  PrintFormat("[LIMIT] KUNLIK ZARAR: %.2f%% >= %.2f%%",
...
double CalcLot(const double riskDist)
  {
   ...
   PrintFormat("[LOT] Balans=%.2f Risk=%.2f SL=%s -> LOT=%.2f (risk %.2f)",
...
bool HasEnoughMargin(const bool isBuy, const double lot, const double price)
  {
   ...
   if(margin > limit)
     {
      PrintFormat("[MARJA] Yetarli emas: kerak=%.2f ruxsat=%.2f", margin, limit);
      return false;
     }
   return true;
  }
...
//==================================================================//
//              ADAPTIV SL BUFFER / RANGE / BREAKOUT FILTRLARI      //
//==================================================================//
double GetBufferPrice(const double spread)
  {
   ...
   if(InpUseATR_Buffer)
     {
      ...
      else { buf = InpSL_BufferPoints * g_pipSize; Log("[BUFFER] ATR yo'q — qat'iy punkt"); }
     }
   else
   ...
   if(buf < minBuf)
     {
      Log(StringFormat("[BUFFER] %s -> %s (spread x%.1f chegarasi)",
...
bool ApproveRangeWidth()
  {
   ...
   if(atrD <= 0.0) { Print("[RANGE-FILTR][OGOH] ATR(D1) yo'q — o'tkazib yuborildi"); return true; }
   ...
   if(g_rangeWidth < minW)
     {
      PrintFormat("[RANGE-FILTR] RAD: juda TOR %s < %s (ATR=%s)",
                  PS(g_rangeWidth), PS(minW), PS(atrD));
      return false;
     }
   if(g_rangeWidth > maxW)
     {
      PrintFormat("[RANGE-FILTR] RAD: juda KENG %s > %s (ATR=%s)",
                  PS(g_rangeWidth), PS(maxW), PS(atrD));
      return false;
     }
   PrintFormat("[RANGE-FILTR] QABUL: %s (ruxsat %s-%s)",
               PS(g_rangeWidth), PS(minW), PS(maxW));
   return true;
  }
   ...
   if(InpMinBreakPct > 0.0)
     {
      ...
      if(pen < need)
        {
         PrintFormat("[BREAK-FILTR] RAD: penetratsiya zaif %s < %s", PS(pen), PS(need));
         return false;
        }
     }
      ...
      if(barRange > 0.0)
        {
         ...
         if(bodyPct < InpMinBodyPct)
           {
            PrintFormat("[BREAK-FILTR] RAD: bar tanasi %.1f%% < %.1f%%",
                        bodyPct, InpMinBodyPct);
            return false;
           }
         bool bullBar = (c > o);
         if(isBuy != bullBar)
           { Print("[BREAK-FILTR] RAD: bar yo'nalishi mos emas"); return false; }
        }
     }
   ...
   if(InpMaxBarsSinceBreak > 0)
     {
      ...
      if(barsOutside > InpMaxBarsSinceBreak)
        {
         PrintFormat("[BREAK-FILTR] RAD: signal eskirgan (%d bar > %d)",
...
//==================================================================//
//          [25][26][27] VAQT BO'YICHA CHIQISH                      //
//==================================================================//
...
//--- [26] Juma kuni majburiy yopish (hafta oxiri gapidan himoya) -----
   if(InpCloseBeforeWeekend && tm.day_of_week == 5 && tm.hour >= InpFridayCloseHour)
      reason = StringFormat("JUMA yopish (%02d:00) — hafta oxiri gap riski",
                            InpFridayCloseHour);

//--- [25] Kunlik majburiy yopish -------------------------------------
   if(reason == "" && InpUseTimeExit && tm.hour >= InpForceCloseHour)
      reason = StringFormat("Kunlik majburiy yopish (%02d:00) — tungi gap riski",
                            InpForceCloseHour);

//--- [27] Max ushlab turish vaqti -------------------------------------
   if(reason == "" && InpMaxHoldHours > 0)
     {
      datetime openTime = (datetime)PositionGetInteger(POSITION_TIME);
      long heldSec = (long)(TimeCurrent() - openTime);
      if(heldSec >= (long)InpMaxHoldHours * 3600)
         reason = StringFormat("Max ushlab turish vaqti: %.1f soat >= %d",
   ...
   PrintFormat("[VAQT-CHIQISH] %s | joriy P/L=%.2f — yopilmoqda", reason, profit);

   if(!trade.PositionClose(ticket))
     {
      PrintFormat("[XATO] Vaqt bo'yicha yopilmadi | retcode=%d | %s",
...
//==================================================================//
//     [22][23][24] POZITSIYA BOSHQARUVI — HAR TICKDA ISHLAYDI      //
//==================================================================//
void ManageOpenPosition()
  {
   ulong ticket = FindOurPosition();

//--- pozitsiya yopilgan -> GV larni tozalash --------------------------
   if(ticket == 0)
     {
...
//--- vaqt bo'yicha chiqish har doim BIRINCHI ---------------------------
...
//--- [28] boshlang'ich SL ni tiklash ----------------------------------
   double initSL;
   if(HasInitialSL(ticket))
      initSL = GetInitialSL(ticket);
   else
     {
      //  Terminal qayta yuklangan bo'lishi mumkin. curSL allaqachon BE ga
      //  ko'chgan bo'lsa, undan risk hisoblash NOTO'G'RI bo'ladi —
      //  shuning uchun bugungi range kengligidan tiklaymiz.
      ...
      PrintFormat("[BOSHQARUV][OGOH] Boshlang'ich SL topilmadi — taxminan tiklandi: %s",
                  PS(initSL));
     }
...
//--- pozitsiya YOPILADIGAN narx (buy -> bid, sell -> ask) -------------
   ...
   if(InpUsePartialClose && profitR >= InpPartialAtR && !IsPartialDone(ticket))
     {
      ...
      if(closeVol >= vmin && remain >= vmin)
         ...
         if(trade.PositionClosePartial(ticket, closeVol))
           {
            ...
            PrintFormat("[QISMAN] +%.2fR da %.2f lot yopildi (qoldi %.2f)",
        ...
        {
         MarkPartialDone(ticket);   // qayta urinmaslik uchun
         PrintFormat("[QISMAN] Imkonsiz: hajm juda kichik (lot=%.2f, min=%.2f)", vol, vmin);
        }
      return;   // shu tickda boshqa o'zgartirish qilmaymiz
     }

//--- [22] BREAK-EVEN va [23] TRAILING ---------------------------------
   double newSL = curSL;
   string what  = "";
...
//--- minimal ko'chirish qadami (order spamiga yo'l qo'ymaslik) ---------
   double minStep = MathMax(_Point, InpTrailStepPoints * g_pipSize);
   bool improved  = isBuy ? (newSL > curSL + minStep) : (newSL < curSL - minStep);
   if(!improved) return;

//--- SL narxdan noto'g'ri tomonda bo'lmasin ---------------------------
   if(isBuy  && newSL >= tick.bid) return;
   if(!isBuy && newSL <= tick.ask) return;

//--- broker STOPS_LEVEL / FREEZE_LEVEL --------------------------------
   double distToPrice = isBuy ? (tick.bid - newSL) : (newSL - tick.ask);
   if(g_stopsLevelPx > 0.0 && distToPrice < g_stopsLevelPx)
     {
      Log(StringFormat("[BOSHQARUV] SL narxga juda yaqin (%s < %s) — kutilmoqda",
                       PS(distToPrice), PS(g_stopsLevelPx)));
      return;
     }
   if(g_freezeLevelPx > 0.0 && distToPrice < g_freezeLevelPx)
     {
      Log("[BOSHQARUV] FREEZE_LEVEL zonasi — o'zgartirish mumkin emas");
      return;
     }
...
int OnInit()
  {
   ...
   if(g_atrDailyH == INVALID_HANDLE || g_atrBufH == INVALID_HANDLE)
     { Alert("XATO: ATR yaratilmadi!"); return(INIT_FAILED); }

   Print("════════════════════════════════════════════════════════");
   Print("  SESSION BREAKOUT GOLD  v3.30  —  BOSQICH 4/5 (BOSHQARUV)");
   Print("════════════════════════════════════════════════════════");
   PrintFormat("  Simvol / TF ......... %s / %s", _Symbol,
               EnumToString((ENUM_TIMEFRAMES)Period()));
   PrintFormat("  Break-even .......... %s",
               (InpUseBreakEven
                ? StringFormat("+%.2fR da, %.2fR qulflanadi", InpBE_TriggerR, InpBE_LockR)
                : "o'chiq"));
   PrintFormat("  Trailing ............ %s",
               (InpUseTrailing
                ? StringFormat("+%.2fR dan, masofa %.2fR", InpTrailStartR, InpTrailDistR)
                : "o'chiq"));
   PrintFormat("  Qisman yopish ....... %s",
               (InpUsePartialClose
                ? StringFormat("+%.2fR da %.0f%%", InpPartialAtR, InpPartialPct)
                : "o'chiq"));
   PrintFormat("  Vaqt chiqishi ....... %s",
               (InpUseTimeExit ? StringFormat("%02d:00", InpForceCloseHour) : "o'chiq"));
   PrintFormat("  Juma yopish ......... %s",
               (InpCloseBeforeWeekend ? StringFormat("%02d:00", InpFridayCloseHour) : "o'chiq"));
   PrintFormat("  Max ushlash ......... %s",
               (InpMaxHoldHours > 0 ? StringFormat("%d soat", InpMaxHoldHours) : "cheksiz"));
   ...
   if(InpUseTimeExit && InpForceCloseHour <= InpSessionEndHour)
     {
      Alert("XATO: InpForceCloseHour savdo oynasi tugashidan KEYIN bo'lishi kerak!");
      return(INIT_PARAMETERS_INCORRECT);
     }
   if(InpUseBreakEven && InpBE_LockR >= InpBE_TriggerR)
     {
      Alert("XATO: InpBE_LockR < InpBE_TriggerR bo'lishi kerak!");
      return(INIT_PARAMETERS_INCORRECT);
     }
   if(InpUseTrailing && InpTrailDistR >= InpTrailStartR)
     {
      Alert("XATO: InpTrailDistR < InpTrailStartR bo'lishi kerak!");
      return(INIT_PARAMETERS_INCORRECT);
     }
   if(InpUsePartialClose && (InpPartialPct <= 0.0 || InpPartialPct >= 100.0))
     {
      Alert("XATO: InpPartialPct 0 va 100 orasida bo'lishi kerak!");
      return(INIT_PARAMETERS_INCORRECT);
     }
      ...
      if(lastWarnDay != todayMidnight)
        {
         ...
         PrintFormat("[RANGE][XATO] Range QURILMADI (bar: %d)", counted);
        }
      return;
     }
   ...
   if(needFix)
     {
      ...
      PrintFormat("[TASDIQ] SL=%s TP=%s risk=%s OK",
                  PS(actualSL), PS(actualTP), PS(realRisk));

//--- [28] boshlang'ich SL ni saqlash (BE/trailing uchun) --------------
...
void TryOpenTrade(const bool isBuy)
  {
   ...
   PrintFormat("│  BE / Trail ..... +%.2fR / +%.2fR", InpBE_TriggerR, InpTrailStartR);
   PrintFormat("└──────────────────────────────────────────────────");
...
void OnTick()
  {
//--- POZITSIYA BOSHQARUVI HAR TICKDA (NewBar gate dan OLDIN!) --------
//    Aks holda BE/trailing/vaqt-chiqish faqat bar boshida ishlardi.
   ManageOpenPosition();
